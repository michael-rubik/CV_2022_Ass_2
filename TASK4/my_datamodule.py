...
class DataModule:
    ...
    def get_transforms(self, train: bool = False):
        ...
        if self.augmented and train:
            ...
            data_transforms = transforms.Compose([transforms.ToTensor(), transforms.RandomSolarize(random.randint(210.0, 240.0), p=0.2), transforms.RandomHorizontalFlip(p=0.5), transforms.ColorJitter(brightness=(0.7, 1), contrast=(0.7, 1), saturation=(0.7,1), hue=0), transforms.RandomRotation(degrees=random.randint(0,33)), transforms.Resize((64, 64))])
